...
@Component({
  selector: 'app-busqueda-figuritas-faltantes',
  templateUrl: './busqueda-figuritas-faltantes.component.html',
  styleUrls: ['./busqueda-figuritas-faltantes.component.css'],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class BusquedaFiguritasFaltantesComponent extends BusquedaFiguritas {

  override pageSize: number = 3
  alerta!: string
  mostrarAlerta: boolean = false

  override figuritaServiceCall(): Promise<Figurita[]> {
    return this.figuritaService.figuritasFaltantesParaAgregar(this.currUserID, this.filtrosAplicados())
  }

  override recibirAlerta(): void {
    this.alertaService.alerta$.subscribe((alerta) => {
      console.log("antes", this.alerta, this.mostrarAlerta)
      this.alerta = alerta
      this.mostrarAlerta = true
      console.log("despues", this.alerta, this.mostrarAlerta)
      setTimeout(() => {
...
export class StubBusquedaFiguritasComponent extends BusquedaFiguritasFaltantesComponent {

  constructor(service: StubFiguritaService, ref: ChangeDetectorRef, alert: AlertasService) { super(service, ref, alert) }

  filterRango() { this.figuritas = this.figuritas.filter(figurita => figurita.valorTotal >= this.filtros.rango.contenido.min && figurita.valorTotal <= this.filtros.rango.contenido.max) }
